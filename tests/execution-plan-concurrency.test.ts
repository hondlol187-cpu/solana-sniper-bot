...
test(
  'cancel rejects already simulated plans',
  async () => {
    ...
    await assert.rejects(
      ...
      /not reusable|simulated|cancelled/
    );
  }
);
...
test(
  'concurrent simulate attempts: only one wins, the other rejects',
  async () => {
    await configureEnvironment();
    ...
    /*
     * Fire two simulate transitions concurrently. Per-plan locking
     * guarantees the second one observes a non-prepared state and
     * rejects — exactly the race the batch is meant to close.
     */
    const results =
    ...
    assert.equal(
      fulfilled.length,
      1,
      'exactly one simulate transition must succeed'
    );
    assert.equal(
      rejected.length,
      1,
      'exactly one simulate transition must reject'
    );
